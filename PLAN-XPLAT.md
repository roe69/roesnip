...
## 7. Linux/cross-platform parity backlog (post-audit, 2026-07-12)

This backlog originated from a user report that the Linux Avalonia build looks and works
noticeably worse than the Windows WPF app, plus a specific complaint about an invisible
Wayland-hotkey failure that only ever surfaced as a stderr message. A 5-way audit was run
comparing WPF vs. Avalonia behavior across the app to catalogue the gap. Three items were fixed
in that same pass: Make the Wayland no-global-hotkey condition visible: one-time toast, persistent
Settings caption, and user-facing docs for the DE-shortcut workaround, Fix the dead X11 black-frame
check: all-zero detection must ignore the force-255 alpha bytes, and Surface the specific
capture-failure reason in the user-facing error instead of the generic 'Capture failed on every
monitor.'. Everything below is the remaining backlog for a future session.

The full ordered WPF-vs-Avalonia parity work plan (which folds these items in alongside the
rest of the eight-way 2026-07-13 audit) now lives in docs/PARITY.md; check items off there.

- **Port the instant-dim latency architecture (FlashDimmer analog + OverlayWindow pooling + startup warmup) to RoeSnip.App** (large) — This is almost certainly the dominant cause of 'works nowhere near as good' — every capture pays cold window construction with zero pre-capture feedback, and the first capture is worst of all. But it cannot be done blind: the WPF design leans on Win32-only primitives (SetWindowPos off-screen parking, WDA_EXCLUDEFROMCAPTURE, raw SetForegroundWindow), Wayland forbids client window positioning outright, and the whole point of the feature is measured latency — which requires a live Linux machine to tune and to confirm the dim doesn't photobomb the portal screenshot. Shipping a guessed port risks regressing the working Windows/macOS Avalonia builds too. Do a design spike first (per-platform park/re-show strategy, capture-exclusion story per compositor), then implement with runtime measurement. The warmup thread alone (no window parking, just pre-JIT of capture/tonemap/window-type) is the safest first slice if a partial step is wanted.
- **Portal robustness: negative-result handling for a persistently unresponsive portal + move the blocking CaptureAll off the UI-adjacent thread** (medium) — Real and nasty (up to 60s of apparent freeze per attempt on pure Wayland with a broken/denied portal), but both halves need runtime validation: the 60s timeout is deliberate (a first-use permission dialog legitimately takes that long, so any negative-cache/timeout-shortening heuristic risks breaking the happy path), and wrapping CaptureAll in Task.Run interacts with Avalonia dispatcher context and the D-Bus session in ways only a live Wayland session can confirm. Pair this with the first live-Linux test session; the error-surfacing doNow item at least makes these failures diagnosable in the meantime.
- **Mixed-DPI multi-monitor portal slicing (per-output scale instead of one global union scale)** (large) — Mathematically confirmed wrong for mixed-scale Wayland setups, but the correct fix requires knowing what XWayland's RandR emulation actually reports per compositor/scale combination — that ground truth is unobtainable without a mixed-DPI Wayland rig, and a guessed formula could break the currently-correct uniform-scale majority case. Needs live hardware; add per-monitor scale diagnostics logging as a cheap first step when someone has a repro machine.
- **Port the screen-recording feature (region MP4/GIF + audio + recording chrome) to Linux/macOS** (large) — ~3,100 lines of Windows-specific code (WGC, Media Foundation, WASAPI COM vtables, WPF GifBitmapEncoder byte-patching) with no portable analog — a Linux port means an all-new PipeWire/ScreenCast-portal capture path and a new encoder stack (ffmpeg or GStreamer). Orders of magnitude beyond one session, and impossible to verify without runtime testing. Should be scoped as its own multi-session project; consider an interim 'Record is Windows-only' note in docs so the gap at least isn't silent.
- **Port the standalone eyedropper/ColorPickerWindow (format catalog, shade strip, recent colors)** (large) — Complete feature loss (~750 lines plus the format-catalog model) but self-contained and UI-heavy — correctness of layout, popover behavior, and capture-exclusion of the picker window all need visual runtime verification that this session can't do. Also never scoped in the porting plan, so it needs a design decision on what the cross-platform capture-exclusion story even is.
- **Per-tool color/width-aware custom cursors in the Avalonia overlay** (medium) — A constant, visible polish regression ('looks much older'), and Avalonia does support bitmap cursors — but the deliverable is purely visual (cursor legibility on arbitrary backgrounds, hotspot alignment, per-DPI sizing) and cannot be judged by compilation. Good candidate for the first session that has screenshot-verification on a live Linux desktop.
- **Auto-update for the Linux AppImage (UpdateManager equivalent)** (large) — A retention/maintenance gap, not a moment-to-moment one, and the risky kind of code to ship unverified: a wrong atomic-swap/relaunch strategy can brick the user's install. Needs an AppImage-specific update strategy decision (appimageupdate conventions vs download-and-replace) and a real end-to-end update test against GitHub Releases. A cheap interim step later: a passive 'new version available' tray notice with a link, no self-swap.
- **Live-Linux smoke test of overlay focus/activation (ForegroundActivator gap) — then decide if any code is needed** (medium) — The auditor themselves flagged this as uncertain: Avalonia's activation-on-show may be entirely sufficient on GNOME/KDE, and Wayland offers no programmatic focus-forcing to escalate to anyway, so writing fallback code blind could be pure dead weight. The action item is a 10-minute manual test ('does the Text tool receive keystrokes immediately after hotkey-opening the overlay'), not code.
- **Share one monitor-enumeration snapshot between the caller and the portal shot (hotplug/resize race)** (small) — Real but narrow (requires a monitor reconfiguration in the instant of a capture) and architecturally entangled with the portal capturer's coalescing design — a careless fix could break the coalescing invariant that is doing real work. Low payoff relative to the items chosen for this session; batch it with the portal-robustness work.
- **Tray tooltip status suffix + no-tray-icon fallback affordance on stock GNOME** (small) — The worst-case discoverability hole (no hotkey + no tray icon) is substantially mitigated by the doNow Wayland toast and README docs, which point at the CLI verbs that always work. The remaining polish (status-reflecting tooltip, detecting a failed StatusNotifier host) is nice-to-have and the detection half needs a live GNOME session to verify.
