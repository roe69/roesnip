...
/// itself uses. Factored out as a static, WPF-rendering-free function specifically so this mapping
/// is testable without a live FrameworkElement/DrawingContext — see the method's own doc comment.</summary>
public class MagnifierSelectionPreviewTests
{
    ...
    public void MapSelectionToLoupeRect_SinglePixelAtCenter_MapsToTheCenterSwatchCell()
    {
        ...
        // identical in kind to how a single swatch is placed in the OnRender grid loop.
    ...
    public void MapSelectionToLoupeRect_UnnormalizedRect_StillProducesANonInvertedRect()
    {
        ...
        // exactly like SelectionAdorner.OnRender does before computing its own dipRect.
    ...
    public void MapSelectionToLoupeRect_FarEdgeOutsideSampleWindow_MapsOutsideTheLoupeSquare_NoBoundsCheck()
    {
        ...
        // itself performs no bounds-checking or clamping — callers (Magnifier.OnRender) are
        // expected to PushClip to the loupe square instead, per the spec's "off-screen edges are
        // simply clipped with no artifact" decision.
