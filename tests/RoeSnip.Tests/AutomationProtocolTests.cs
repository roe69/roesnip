...
/// App/AutomationServer.cs's own doc comment) — no live window, no Dispatcher, no pipe. The live
/// half (actually driving OverlayController/RecordingController) is verified end-to-end against a
/// resident process, not here — see TESTING.md's "Driving RoeSnip programmatically" section.</summary>
public class AutomationProtocolTests
{
    ...
    [Theory]
    [InlineData(5)]   // range floor
    [InlineData(50)]  // GIF's own ceiling — still inside the pure layer's 5-60 union range
    [InlineData(37)]  // not a divisor of 100 — legal now that fps is a free integer, not four chips
    [InlineData(60)]  // MP4's own (and the union's) ceiling
    ...
    public void BuildError_ProducesOkFalseWithTheMessage()
    {
        ...
        var request = AutomationProtocol.TryParseRequest(json, out _);
        // BuildError's output isn't itself a "cmd" request, but it must still be valid JSON with the
        // documented ok/error shape - parse it directly instead of through TryParseRequest.
    ...
    [Fact]
    public void SerializeState_RecordingSetup_IncludesFormatPresetEstimateAndFps()
    {
        var state = new AutomationProtocol.StateSnapshot(
            "setup", new AutomationProtocol.SelectionDto(0, 0, 640, 480),
            "gif", "quality", "~700 KB/s * 41 MB/min (varies with motion)",
            25, new AutomationProtocol.FpsRangeDto(5, 50),
            new List<AutomationProtocol.MonitorDto>());
        string json = AutomationProtocol.SerializeState(state);

        Assert.Contains("\"recordingFormat\":\"gif\"", json);
        Assert.Contains("\"preset\":\"quality\"", json);
        Assert.Contains("varies with motion", json);
        Assert.Contains("\"fps\":25", json);
        Assert.Contains("\"fpsRange\":{\"min\":5,\"max\":50}", json);
    }
    ...
    {
        var state = new AutomationProtocol.StateSnapshot(
            "setup", null, "mp4", "max", "~1 MB/s * 60 MB/min", 30, new AutomationProtocol.FpsRangeDto(5, 60),
