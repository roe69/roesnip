...
/// <summary>Ported from the WPF app's tests/RoeSnip.Tests/AutomationClientTests.cs (keeps the
/// 3ecb895 disposal-order fix's regression coverage): a real resident commonly tears its end of the
/// pipe down the instant after flushing the response line (a fast request/response cycle), which
/// made <c>AutomationClient.Run</c>'s old `using var` cleanup throw "Cannot access a closed pipe"
/// from INSIDE its own try block — caught by the generic `catch (Exception)`, stomping an
/// already-correct, already-printed ok:true response with a spurious exit code 1. This reproduces
/// that exact abrupt-close shape directly against a bare <see cref="NamedPipeServerStream"/> (never
/// a live RoeSnip.App resident, never the single-instance mutex/pipe) and asserts the exit code
/// tracks the response's own "ok" field regardless of what the server does to its end immediately
/// after replying.</summary>
public class AutomationClientTests
{
    /// <summary>Starts a one-shot server on a private, per-call pipe name (never the real
    /// "RoeSnip.App-Automation" name — a resident running with --automation owns that with
    ...
    {
        string pipeName = $"RoeSnip.App-Automation-Test-{Guid.NewGuid():N}";
