...
/// <summary>Pure geometry for the cross-monitor ("spanning") selection feature — ported from the
/// frozen WPF app's tests/RoeSnip.Tests/SpanningSelectionMathTests.cs (item 09 of the xplat port).
/// Covers both the distribute primitive (used by every drag mode: NewSelection, and
/// SpanningResize/SpanningMove) and ApplyResize, the resize math those last two now share with the
/// plain single-monitor Resize drag. Only WPF's System.Windows.Point usages became plain (x, y)
/// doubles — RoeSnip.Core has no UI-framework dependency (see SpanningSelectionMath's own doc
/// comment).</summary>
public class SpanningSelectionMathTests
{
    ...
    {
        var result = SpanningSelectionMath.ApplyResize(ResizeStart, SelectionHandle.Right, 250, 999);
    ...
    {
        var result = SpanningSelectionMath.ApplyResize(ResizeStart, SelectionHandle.TopLeft, 50, 60);
    ...
    public void ApplyResize_DraggedPastTheOppositeCorner_NormalizesInsteadOfStayingInverted()
    {
        ...
        var result = SpanningSelectionMath.ApplyResize(ResizeStart, SelectionHandle.TopLeft, 500, 500);
    ...
    public void ApplyResize_Body_LeavesTheRectUnchanged()
    {
        ...
        var result = SpanningSelectionMath.ApplyResize(ResizeStart, SelectionHandle.Body, 999, 999);
