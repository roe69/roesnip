...
/// <summary>Ported from tests/RoeSnip.Tests/ToneMapperTests.cs (PLAN-XPLAT.md §3.1). One mechanical
/// update: every synthetic CapturedFrame now also supplies
/// <c>sdrWhiteInBufferUnits: sdrWhiteNits / 80.0</c> (the Windows scRGB convention), which keeps
/// every golden numeric assertion valid unchanged per the algebraic identity in PLAN-XPLAT.md
/// §2.2/§2.5. Plus one NEW test exercising the macOS-EDR-style convention
/// (SdrWhiteInBufferUnits == 1.0).</summary>
public class ToneMapperTests
{
    ...
    private static CapturedFrame MakeFp16Frame(
        ...
        int? strideOverride = null, double? sdrWhiteInBufferUnitsOverride = null)
    {
        ...
        // Windows scRGB convention unless a test overrides it (the macOS EDR convention test does).
        double sdrWhiteInBufferUnits = sdrWhiteInBufferUnitsOverride ?? monitor.SdrWhiteNits / 80.0;
        return new CapturedFrame(FrameFormat.Fp16ScRgb, width, height, stride, bytes, monitor, sdrWhiteInBufferUnits);
    }
    ...
    [Fact]
    public void EdrConvention_SdrWhiteInBufferUnitsOne_ShouldersAndPassesThroughCorrectly()
    {
        // NEW in the Core port (PLAN-XPLAT.md §3.1): the macOS-EDR-style convention, where 1.0
        // buffer unit IS SDR white (SdrWhiteInBufferUnits = 1.0) rather than Windows scRGB's
        // 80-nits-per-unit rule. A raw buffer value of 2.5 is 2.5x SDR white; a raw 0.5 is plain
        // SDR mid-bright. Per the algebraic identity in PLAN-XPLAT.md §2.2, this frame must be
        // byte-identical to a Windows-convention frame carrying the same post-scale values
        // (SdrWhiteNits=240 => SdrWhiteInBufferUnits=3.0; raw values 7.5 and 1.5) on the same
        // monitor — proving the generalized 1/SdrWhiteInBufferUnits scale step, not just the
        // Windows-equivalent path, shoulders correctly.
        var monitor = MakeMonitor(sdrWhiteNits: 240.0, maxLuminanceNits: 1000.0);

        var edrFrame = MakeFp16Frame(2, 1,
            (x, _) => x == 0 ? (2.5f, 2.5f, 2.5f, 1.0f) : (0.5f, 0.5f, 0.5f, 1.0f),
            monitor, sdrWhiteInBufferUnitsOverride: 1.0);
        var windowsFrame = MakeFp16Frame(2, 1,
            (x, _) => x == 0 ? (7.5f, 7.5f, 7.5f, 1.0f) : (1.5f, 1.5f, 1.5f, 1.0f),
            monitor); // default: sdrWhiteInBufferUnits = 240/80 = 3.0

        var edrImage = ToneMapper.MapToSdr(edrFrame, new ToneMapOptions());
        var windowsImage = ToneMapper.MapToSdr(windowsFrame, new ToneMapOptions());

        var (bEdr0, gEdr0, rEdr0, _) = ReadOutputPixel(edrImage, 0, 0);
        var (bEdr1, gEdr1, rEdr1, _) = ReadOutputPixel(edrImage, 1, 0);
        var (bWin0, gWin0, rWin0, _) = ReadOutputPixel(windowsImage, 0, 0);
        var (bWin1, gWin1, rWin1, _) = ReadOutputPixel(windowsImage, 1, 0);

        // 2.5x SDR white is the frame max AND the (derived, clamped) peak => maps to full white.
        Assert.Equal(255, rEdr0);
        Assert.Equal(255, gEdr0);
        Assert.Equal(255, bEdr0);
        // 0.5 (below knee) is a pass-through pixel: the DESIGN.md golden pair linear 0.5 -> byte 188.
        Assert.Equal(188, rEdr1);
        Assert.Equal(188, gEdr1);
        Assert.Equal(188, bEdr1);
        // And the whole EDR-convention output equals the Windows-convention equivalent, per pixel.
        Assert.Equal((bWin0, gWin0, rWin0), (bEdr0, gEdr0, rEdr0));
        Assert.Equal((bWin1, gWin1, rWin1), (bEdr1, gEdr1, rEdr1));
    }

    [Fact]
    public void ReadPixelNits_Fp16EdrConvention_UsesSdrWhiteInBufferUnits()
    {
        // EDR frame: raw 2.0 with SdrWhiteInBufferUnits=1.0 on a 240-nit-SDR-white monitor reads
        // as 2.0 * 240 = 480 nits.
        var monitor = MakeMonitor(sdrWhiteNits: 240.0);
        var frame = MakeFp16Frame(1, 1, (_, _) => (2.0f, 2.0f, 2.0f, 1.0f), monitor,
            sdrWhiteInBufferUnitsOverride: 1.0);

        Assert.Equal(480.0, frame.ReadPixelNits(0, 0), precision: 3);
    }

    [Fact]
    public void ReadPixelNits_Bgra8Srgb_WhiteReadsAsMonitorSdrWhite_NotSquared()
    {
        // PLAN-XPLAT.md §2.2's documented Bgra8Srgb rule: the sRGB branch keeps the WPF app's exact
        // constant-*80.0 math (ReadPixelScRgb already bakes SdrWhiteNits/80 into the decode), so a
        // pure-white byte reads back as exactly Monitor.SdrWhiteNits — NOT SdrWhiteNits²/80, which
        // is what naively routing this format through SdrWhiteInBufferUnits would produce.
        var monitor = MakeMonitor(sdrWhiteNits: 240.0);
        var frame = new CapturedFrame(FrameFormat.Bgra8Srgb, 1, 1, 4, new byte[] { 255, 255, 255, 255 },
            monitor, sdrWhiteInBufferUnits: 1.0);

        Assert.Equal(240.0, frame.ReadPixelNits(0, 0), precision: 2);
    }
}
