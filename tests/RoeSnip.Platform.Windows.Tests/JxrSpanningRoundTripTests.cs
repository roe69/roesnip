...
/// <summary>Cross-monitor selection (item 09) HDR save (JxrWriter.WriteSpanning): stitching raw
/// scRGB crops from multiple monitors into one .jxr is well-defined because raw scRGB is one
/// absolute linear space (1.0 = 80 nits) regardless of which monitor a pixel came from — see
/// WriteSpanning's own doc comment. These tests build two small synthetic CapturedFrames with
/// distinct, known pixel values, place them at different offsets in a shared virtual canvas via
/// SpanningFrameCrop, and verify the written file's decoded pixels land exactly where expected —
/// including the documented gap fill (opaque linear black) for canvas pixels no crop covers.
///
/// Ported from tests/RoeSnip.Tests/JxrSpanningRoundTripTests.cs (PLAN-XPLAT.md §3.2) — same
/// mechanical edits as JxrRoundTripTests.cs (Core's portable MonitorInfo shape, CapturedFrame's
/// sdrWhiteInBufferUnits argument). Same WIC-direct decode helper as JxrRoundTripTests (a WPF/
/// Avalonia decoder flattens float headroom regardless of what's actually in the file).</summary>
public class JxrSpanningRoundTripTests
{
    ...
    public void WriteSpanning_Bgra8SrgbCrop_DoesNotThrow_AndProducesFile()
    {
        ...
        var frame = new CapturedFrame(
            FrameFormat.Bgra8Srgb, 1, 1, 4, new byte[] { 200, 150, 100, 255 }, monitor,
