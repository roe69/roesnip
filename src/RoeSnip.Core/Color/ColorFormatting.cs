...
/// <summary>Pure string-formatting / color-space conversions for the eyedropper's format rows
/// (HEX/RGB/HSL/NITS) and its lightness-shades strip. No UI-framework dependency, so it's
/// trivially unit-testable. Ported verbatim from the frozen WPF app's own
/// src/RoeSnip/Overlay/ColorFormatting.cs (item 22) — the WPF app keeps its own independent copy
/// (same "move logic into Core, leave the frozen app's copy alone" pattern every prior porting
/// item has used), only RoeSnip.App's eyedropper/magnifier consume this one.</summary>
public static class ColorFormatting
{
    /// <summary>Lowercase, no leading '#' — matches the eyedropper's HEX row exactly (e.g.
    /// "0d1117"). This is a display format, distinct from the "#RRGGBB" storage format used by
    /// settings persistence (RecentPickedColors/CustomColors) and the rest of the app's existing
    ...
    /// copies and the one used for settings storage (RecentPickedColors/CustomColors), so it
    /// round-trips cleanly through a "#RRGGBB" color parser on either platform.</summary>
