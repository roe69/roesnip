...
/// <summary>Pure, UI-framework-free action-based undo/redo history — the model an overlay's shape
/// list is built on. Generic over the shape type purely so this file has zero dependency on
/// System.Windows/Avalonia types (WPF's AnnotationShape is built from System.Windows.Point/Color,
/// Avalonia's from Avalonia.Point/Media.Color) and is unit-testable in isolation. Ported verbatim
/// from the WPF app's src/RoeSnip/Overlay/AnnotationHistory.cs into RoeSnip.Core so both the WPF
/// and Avalonia AnnotationLayer can share one implementation.
///
/// Three action kinds cover everything an AnnotationLayer needs: <see cref="Add"/> (a newly
/// committed shape), <see cref="Remove"/> (a deleted shape — remembers its list index so Undo
/// re-inserts it in the same position instead of at the end, which would silently reorder
/// draw/overlap order), and <see cref="Replace"/> (an existing shape's list slot swapped for a
/// mutated version — move/resize drags, wheel font-resize, and text re-edit commits all funnel
/// through this one). Undo pops the most recent action and applies its inverse; Redo re-applies
/// it. Any new action clears the redo stack, exactly like the two-list Undo/Redo stack this
/// replaces.</summary>
