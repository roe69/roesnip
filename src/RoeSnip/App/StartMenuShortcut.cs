...
/// Install() puts the exe under %LOCALAPPDATA%\RoeSnip, which Windows Search does not index - an
/// installed RoeSnip is invisible to Start-menu search unless a shortcut exists under the user's
/// Start Menu\Programs folder (the thing search actually indexes). Self-updates swap the exe at the
/// same path, so a once-created shortcut stays valid across updates; <see cref="EnsureFor"/> is
/// also called at startup (not just from Install) so installs that predate this feature gain the
/// shortcut on their first launch after updating. Same convenience-never-load-bearing contract as
/// the rest of the install/update machinery: never throws, and like UpdateManager it is reviewed
/// by eye rather than unit-tested (real COM + real filesystem).</summary>
public static class StartMenuShortcut
{
    public static string ShortcutPath { get; } =
        Path.Combine(Environment.GetFolderPath(Environment.SpecialFolder.Programs), "RoeSnip.lnk");
